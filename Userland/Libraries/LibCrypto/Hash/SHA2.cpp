...
__attribute__((target("default"))) static void SHA256_transform_impl(u32* state, u8 const* data)
{
    u32 m[64];
...
#if ARCH(I386) || ARCH(X86_64) && !defined KERNEL
__attribute__((target("sse2,ssse3,sse4.1,sha"))) static void SHA256_transform_impl(u32* state, u8 const* data)
{
    ...
    state_0 = _mm_loadu_si128(reinterpret_cast<__m128i const*>(&state[0]));
    state_1 = _mm_loadu_si128(reinterpret_cast<__m128i const*>(&state[4]));
    tmp = _mm_shuffle_epi32(state_0, 0xb1);
